...
const LABELS: Record<string, string> = {
  ...
  recent_reason: '最近原因', intimacy: '亲密度', respect: '尊重', fear: '畏惧', loyalty: '忠诚',
}
  ...
  return (
    <section className="runtime-card">
      <div className="runtime-section-title">
        ...
        <span className={`tag ml-auto ${activity.changed ? 'tag-gold' : ''}`}>{activityLabel(activity)}</span>
      </div>
      {!changed ? (
        ...
        <div className="space-y-3">
          ...
          {reason ? <p className="runtime-note">变化原因：{reason}</p> : <p className="runtime-note">来源：角色卡变量或本轮结构化状态补丁。</p>}
        </div>
      )}
    </section>
  )
