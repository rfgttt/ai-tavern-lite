...
def test_profile_v4_backfills_json_initvar_into_existing_session(test_db):
    from app.db.models import SessionState
    ...
    normalized = {
        ...
        'character_book': {'entries': [
            ...
            {'comment': '变量规则', 'enabled': True, 'content': '<UpdateVariable>_.add</UpdateVariable>'},
        ]},
    }
    character = Character(
        name='穗秋生',
    ...
    test_db.add(character)
    test_db.flush()
    session = ChatSession(character_id=character.id, title='旧 JSON 会话')
    test_db.add(session)
    test_db.flush()
    runtime = SessionState(
        session_id=session.id,
        profile_json=json.dumps({'version': 3, 'compatibility_core': 'tavern-safe-v2'}),
        initial_state_json=json.dumps({'custom': {}, 'character': {'name': '穗秋生'}, 'relationship': {}}),
        state_json=json.dumps({'custom': {}, 'character': {'name': '穗秋生'}, 'relationship': {}}),
        revision=12,
    )
    test_db.add(runtime)
    test_db.flush()
    ...
    migrated = ensure_session_state(test_db, session, character)
    current = json.loads(migrated.state_json)
    profile = json.loads(migrated.profile_json)

    assert profile['version'] == 8
    assert profile['compatibility_core'] == 'tavern-safe-v6'
    assert current['custom']['穗秋生']['好感度'][0] == 5
    assert current['relationship']['affection'] == 5
    assert current['relationship']['fear'] == 95
    assert current['relationship']['dependence'] == 100
    assert current['scene']['location'] == '旧住宅'
    assert current['character']['important_memories'] == []
